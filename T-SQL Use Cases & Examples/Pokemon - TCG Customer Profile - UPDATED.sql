...
select
	*,
	case when bookinglocationid = 4 then 2239 else storeid end 'storeid2',
	case when bookinglocationid = 10 then 3313 else storeid end 'storeid3'
into
	#BELONG_STORE
from
	bookinglocation (nolock)
where
	bookinglocationid not in (7,23,28,31)
go	
...
where
	tc.transactiondate >= cast(dateadd(month,-18,getdate()) as date)							-- Pokemon Base
...
from
	...
	inner join transactions tc (nolock) on emc.insightcustomerid = tc.insightcustomerid
where
	tc.transactiondate >= cast(dateadd(month,-18,getdate()) as date)							-- GAME Base
	and tc.storenumber != 9999
	and emc.mergedcustomerid not in (select mergedcustomerid from #TARGETCUST)
group by
	emc.mergedcustomerid
go
...
select
	...
	(0 + convert(char(8),cast(getdate() as date),112) - convert(char(8),cc.dateOfBirth,112)) / 10000 [age],
	cc.title,
	cc.gender,
	ic.email,
	ic.rewardnumber
into
	#CUSTGROUP
from
	#BASE x  -- Change to selected target group
...
from
	#BELONG_STORE bs
	inner join transactions tc (nolock)	on bs.storeid = tc.storenumber or bs.storeid2 = tc.storenumber or bs.storeid3 = tc.storenumber
where
