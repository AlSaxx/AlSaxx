...
if object_id ('tempdb..#BELONG_STORE') is not NULL
	drop table #BELONG_STORE
select
	*
into
	#BELONG_STORE
from
	bookinglocation (nolock)
where
	bookinglocationid not in (23,28,7,31)
union all
select
	distinct bookinglocationid,
	addressline1,
	addressline2,
	addressline3,
	city,
	...
	createdDate,
	case 
		when bookinglocationid = 4 then 593
		when bookinglocationid = 10 then 3313
	end 'storeid'
from
	bookinglocation (nolock)
where
	bookinglocationid in (4,10)
go	
...
into
	#CUSTGROUP
from
	#TARGETCUST x  -- Change to selected target group
...
if object_id ('tempdb..#BELONGSTORETRANS') is not NULL
	drop table #BELONGSTORETRANS
select
	bs.*,
	tc.id 'tid',
	tc.insightcustomerid,
	tc.transactiondate
into
	#BELONGSTORETRANS
from
	#BELONG_STORE bs
	inner join transactions tc (nolock)	on tc.storenumber = bs.storeid
where
	tc.transactiondate >= bs.createdDate
go


if object_id ('tempdb..#BELONG_CUST_LTD') is not NULL
	drop table #BELONG_CUST_LTD
select
	distinct emc.mergedcustomerid,
	min(bst.transactiondate) 'minTrans'
into
	#BELONG_CUST_LTD
from
	expanededmergedcustomerids emc (nolock)
	inner join #BELONGSTORETRANS bst (nolock) on emc.insightcustomerid = bst.insightcustomerid
	inner join transactionitem ti (nolock) on bst.tid = ti.transactionid
	inner join sku s (nolock) on ti.sku = s.sku
where
	s.departmentid = 340
group by
...
if object_id ('tempdb..#BELONG_CUST') is not NULL	
	drop table #BELONG_CUST
select
	distinct mergedcustomerid,
	minTrans 'minTRGTrans'
into
	#BELONG_CUST
from
	#BELONG_CUST_LTD
where
	mergedcustomerid not in (select mergedCustomerId from insighttemp..GK_CUSTOMER_BLACKLIST)
go
...
union all
select
	'Belong Customer' 'Measure',
	count (distinct x.mergedcustomerid) 'Data'
from
	#CUSTGROUP x
	inner join #BELONG_CUST bc (nolock) on x.mergedcustomerid = bc.mergedcustomerid
union all
select
	'New to Belong' 'Measure',
	count (distinct x.mergedcustomerid) 'Data'
from
	#CUSTGROUP x
	inner join #BELONG_CUST mt on x.mergedcustomerid = mt.mergedcustomerid
where
	x.minTRGTrans = mt.minTRGTrans	
go
...
if object_id ('tempdb..#ConsoleTable') is not NULL	
...
select [Platform], isnull(t1.[Format Preference],0) FormatPreference from #ConsoleTable ct
left join (
select
	distinct [FORMAT],
...
group by
	[FORMAT]
) T1 on t1.FORMAT = ct.Platform
order by ct.Platform
go


if object_id ('tempdb..#CatagoryTable') is not NULL	
	drop table #CatagoryTable
create table #CatagoryTable
(
[Platform] nvarchar(20)
)

Insert into #CatagoryTable
values ('Board Games'),
	   ('Clothing'),
	   ('TCG'),
	   ('Tech'),
...
select [Platform], isnull(t1.[Format Preference],0) CategoryPreference from #CatagoryTable ct
left join (
select
	distinct [FORMAT],
	count (distinct mergedcustomerid) 'Format Preference'
...
if object_id ('tempdb..#GenreTable') is not NULL	
	drop table #GenreTable
...
values ('Education and Problem Solving'),
	   ('Shooting'),
	   ('Action/Adventure'),
	   ('Dancing/Music'),
	   ('Horror/Drama/Crime'),
...
select [Platform], isnull(t1.[Genre Preference],0) GenrePreference from #GenreTable ct
left join (
select
	distinct Genre,
...
if object_id ('tempdb..#SegmentTable') is not NULL	
...
values (1, 'GAME Rookies'),
	   (2, 'Casual Acquaintance'),
	   (3, 'Pocket Regulars'),
	   (4, 'Occasional Trippers'),
	   (5, 'Single Splurgers'),
	   (6, 'Average Shoppers'),
	   (7, 'Middleweight Spenders'),
	   (8, 'Big Time Spenders'),
	   (9, 'Top Elite')

select  ct.SegmentID, ct.SegmentName , isnull(t1.cust,0) Customers from #SegmentTable ct
left join (
select
...
group by
	VFSegmentID,
	VFSegmentName
) T1 on t1.VFSegmentID = ct.SegmentID
order by ct.SegmentID
go
...
if object_id ('tempdb..#PoPSegmentTable') is not NULL	
	drop table #PoPSegmentTable
create table #PoPSegmentTable
(
[SegmentID] int,
[SegmentName] nvarchar(30)
)

Insert into #PoPSegmentTable
values (1, 'GAME Rookies'),
	   (2, 'Casual Acquaintance'),
	   (3, 'Pocket Regulars'),
	   (4, 'Occasional Trippers'),
...
select  ct.SegmentID, ct.SegmentName , isnull(t1.cust,0) Customers from #PoPSegmentTable ct
left join (
select
...
group by
	VFSegmentID,
	VFSegmentName
) T1 on t1.purchaseSegmentID = ct.SegmentID
order by ct.SegmentID
go



if object_id ('tempdb..#BelongSegmentTable') is not NULL	
	drop table #BelongSegmentTable
create table #BelongSegmentTable
(
[SegmentID] int,
[SegmentName] nvarchar(50)
)

Insert into #BelongSegmentTable
values (0, 'Noobs'),
	   (1, 'Competition & Community'),
	   (2, 'Fun with Friends / Private Player'),
	   (4, 'Time Together / Grateful Guardians')

select  ct.SegmentID, ct.SegmentName , isnull(t1.cust,0) Customers from #BelongSegmentTable ct
left join (
select
...
group by
	BelongSegmentID,
	BelongSegmentName
) T1 on t1.BelongSegmentID = ct.SegmentID
order by ct.SegmentID
go
